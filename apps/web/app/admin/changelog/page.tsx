...
const BATCHES: Batch[] = [
  {
    ...
    items: [
      ...
      { title: "POST-DEPLOY: prod login returned 'Credential account not found' even though credential rows were backfilled (verified 14 rows). Three follow-up fixes: (1) commit 73fb993 — added User.updatedAt + mapped createdAt to existing createdDate column (better-auth's user schema requires both as REQUIRED fields). (2) commit d1f08ed — Prisma client Proxy that mirrors `userId` → `id` on every prisma.user.find* result, because better-auth's getDefaultFieldName hard-codes `if (field === \"id\") return \"id\"` and ignores the `user.fields.id` config option. (3) commit 436d012 — same proxy now also rewrites `where: { id: ... }` → `where: { userId: ... }` for the inverse direction (findUserById was failing with PrismaClientValidationError 'Unknown argument id'). Final smoke: live login + cookie + /auth/me + /profile/edit all working" },
    ],
  },
  {
    id: "phase-16-2",
