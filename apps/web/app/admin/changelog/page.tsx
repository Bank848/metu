...
const BATCHES: Batch[] = [
  {
    id: "phase-52",
    title: "Phase 52 — codebase tidy + custom domain + email branding",
    subtitle: "Domain swap to metu.online, BFF route consolidation, env-config centralisation, comment-noise sweep.",
    icon: Wrench,
    tone: "info",
    shippedAt: "May 4, 2026",
    commitSha: "2c7d533",
    items: [
      { title: "Custom domain metu.online wired up at Porkbun + Cloudflare-issued Let's Encrypt cert via Fly. Old metu.fly.dev now 301s to the canonical host through a new middleware redirect — bookmarks survive, search engines stop indexing the bare Fly URL." },
      { title: "Resend domain verified at metu.online (DKIM + SPF + DMARC) so receipt emails ship from noreply@metu.online instead of the sandbox sender. Buyers who aren't the Resend account owner now get the email." },
      { title: "BFF route consolidation: 84 forwarder files collapsed into 13 + a single /api/[...path]/route.ts catch-all. Routes with real BFF logic (login cookie, register cookie, checkout revalidate, profile export) stay individual. Net diff: -1,100 lines." },
      { title: "Centralised env reads in apps/server/src/config.ts + apps/web/lib/config.ts. Site URL / API URL / Turnstile / Stripe keys all read from one place — a domain change is now one edit, not a grep-and-pray across 16 files." },
      { title: "Skeleton primitives (PageHeaderSkeleton, FilterBarSkeleton, KpiTilesSkeleton, DataTableSkeleton) extracted from the 40-file loading.tsx pile. Admin pages (users / stores / audit / refunds / reports / security) now compose primitives instead of hand-rolling Skeletons." },
      { title: "Comment-noise sweep: ~390 files touched, ~330 lines of \"Phase X — ...\" / \"Audit follow-up — ...\" preambles deleted. The why for each subsystem now lives in its commit message, not as a smell on every function." },
    ],
  },
  {
    id: "phase-51",
    title: "Phase 51 — security hardening + regression suite",
    subtitle: "Two QA passes closed 17 bugs total. New regression tests guard every fix.",
    icon: Shield,
    tone: "danger",
    shippedAt: "May 3, 2026",
    commitSha: "0ba252a",
    items: [
      { title: "Phone OTP + license keys swapped from Math.random to crypto.randomInt. Math.random is Mersenne Twister — not CSPRNG — so a leaked seed reveals every in-flight OTP and lets a piracy ring predict license keys from a single legit purchase." },
      { title: "Stripe webhook now verifies amount_received matches order.totalPrice * 100 before flipping status to paid. On mismatch the order stays pending and a stripe.amount_mismatch audit row gets written for manual review. Defence in depth so a tampered PaymentIntent can't auto-fulfil." },
      { title: "Seller CSV export neutralises spreadsheet formula injection. A buyer with a username like =cmd|'/c calc'!A0 used to execute as a formula when the seller opened the CSV in Excel/Sheets — escapeCsv now prefixes any cell starting with =, +, -, @, tab, or CR with a single quote." },
      { title: "POST /auth/register catches the P2002 unique-constraint race: when two parallel registrations slip past the dup pre-check, the DB rejects the second insert and we map it to a clean 409 EmailTaken / UsernameTaken instead of a 500." },
      { title: "Cart already-owned guard blocks re-adding a non-stackable product (download / streaming / email delivery) the buyer has already paid for. Mirror guard at checkout catches buyers who pre-loaded their cart before the gate shipped. license_key products stay stackable so bulk-key purchases still work." },
      { title: "Multi-store checkout blocked when any store has Stripe live (silently fell back to demo mode = free goods). Zero-total orders rejected so a $0 fixed coupon can't drain the catalogue. Stale pending orders (>30 min) auto-cancelled with stock restoration to prevent inventory-DoS." },
      { title: "Coupon usage limits enforced inside the order $transaction. The unique (couponId, userId) index handles per-user dedup; an internal count() handles the global usage limit. P2002 on the dedup row maps to a clean 400 CouponAlreadyUsed." },
      { title: "Reviews require a paid order on the product (verified-purchase gate, 403 MustPurchaseToReview) + one review per (user, product) pair (409 AlreadyReviewed). Sanitises HTML tags + control chars from comment text before insert." },
      { title: "OTP brute-force protection: each wrong guess shortens the TTL by 2 min. After ~5 wrong guesses the OTP auto-expires. Generic InvalidCode error for every failure mode so attackers can't tell whether the email is registered or whether a recent OTP exists." },
      { title: "Turnstile (Cloudflare CAPTCHA) added to login + forgot-password (already on register). Dodges credential-stuffing bots that distribute across IPs to slip past the per-IP rate limit. Skipped on the admin-OTP round-trip so submitting the code doesn't re-render the widget." },
      { title: "Self-delete blocked while a Stripe PI is pending (stops the webhook from emailing deleted_<id>@deleted.invalid)  and when the user is the sole admin (LastAdminCannotBeRemoved)." },
      { title: "Seller refund + admin refund now actually call the Stripe Refunds API. Previously they only updated the DB ledger — the buyer's card was never credited. New 502 path surfaces Stripe API errors instead of silently flipping order to refunded." },
      { title: "Regression suite: 33 new tests across 4 files (phase51-utils, phase51-checkout, phase51-auth, stripe-webhook) covering every fix above. Total suite now 184 server + 37 web = 221 tests, all green." },
    ],
  },
  {
    id: "phase-50",
    title: "Phase 50 — QA pass: cart cap, price sort, sitemap, rating aggregate",
    subtitle: "Four bugs from a focused source-code review. Each one was ship-stopping for the audit.",
    icon: Bug,
    tone: "warning",
    shippedAt: "May 3, 2026",
    commitSha: "134187f",
    items: [
      { title: "Cart/checkout availability gate: a buyer who held the cart open through a seller pause / store suspension / soft-delete used to push through anyway. addItem / updateItem / checkout now re-validate every line against loadPurchasableProductItem on every write — over-cap quantities reject with QuantityExceedsCap." },
      { title: "Price sort applied AFTER pagination instead of before — page 1 of /browse?sort=price_asc was sorting only its 12 rows. Now uses a Postgres LATERAL join to compute MIN(unit price) per product across variants, sorts the FULL set, then paginates." },
      { title: "Static sitemap.xml had hardcoded product IDs from the demo seed. Now it queries Prisma at request time for live products + stores so /sitemap.xml never 404s on a real link." },
      { title: "Product rating aggregate read from the most recent 100 reviews instead of the full set — a product with 200 reviews showed a stale 4.2 even after 50 fresh 5-stars. Now aggregates all reviews via a single $queryRaw with COUNT + AVG." },
    ],
  },
  {
    id: "phase-49",
    title: "Phase 49 — admin OTP gate, trusted devices, single-session",
    subtitle: "The public seed admin@metu.dev account is now far harder to compromise even when the password is known.",
    icon: KeyRound,
    tone: "purple",
    shippedAt: "May 2, 2026",
    commitSha: "34a837c",
    items: [
      { title: "Guarded accounts (admin@metu.dev by default — anyone with the URL can attempt to sign in) now require a 6-digit code emailed to the registered address before the better-auth cookie is issued. Two-step login: password first, then OTP + 'I confirm I own this account' checkbox." },
      { title: "Trusted-device cookie: ticking 'trust this device for 7 days' alongside a successful OTP mints a signed cookie so the next sign-in skips the OTP gate. Per-device, expires automatically, revocable from /profile/sessions." },
      { title: "Single-session enforcement: every successful sign-in drops every OTHER session row for that user, so a concurrent login from a stolen cookie kicks the previous tab to /login on its next request. Heuristic: newest createdAt for the user is ours." },
      { title: "trusted_device migration is idempotent — IF NOT EXISTS guards on the table + index so re-running migrate deploy on prod doesn't fail when the table already exists from a partial earlier deploy." },
    ],
  },
  {
    id: "phase-48",
    title: "Phase 48 — already-owned guard, IP ban, GDPR self-delete, useful admin actions",
    subtitle: "A blast of feature work driven by the pre-defense decision plan. Schema impact: one new BannedIp table.",
    icon: Layers,
    tone: "yellow",
    shippedAt: "May 2, 2026",
    commitSha: "c414759",
    items: [
      { title: "Already-owned guard: Product.isStackable (default true for license_key, false for download/streaming/email) blocks re-purchase of one-shot digital goods. Cart shows a 'View order #X' chip; checkout rejects with 409 AlreadyOwned." },
      { title: "Hybrid user-remove flow: fresh accounts hard-delete via prisma.user.delete (cascade clears cart/session/account/favourite); accounts with order/review/transaction history get anonymised (PII blanked, deletedAt set, sessions dropped) so seller analytics survive. Bans are a separate flow that keeps the row + 'Banned' badge for moderation accountability." },
      { title: "GDPR self-delete: DELETE /auth/me requires the user to retype their username as confirmation. Routes through the same hybrid logic. Last-admin guard prevents the sole admin from locking themselves out." },
      { title: "Banned IPs table + middleware that runs FIRST in the chain (before auth, before body parsing). 60-second cache so the DB sees one read per banned IP per minute. Admin /security page lists / adds / removes; row action 'Ban this user's IPs' iterates the user's session rows." },
      { title: "Make seller auto-creates an empty placeholder store (or restores a soft-deleted one). Make buyer soft-deletes the active store. Force password reset emails the user out-of-band. Each row action now does its full side-effect chain instead of just flipping a flag." },
      { title: "Banned user kick-out: dropping the better-auth Session row alongside the deletedAt + bannedAt flip so the next request 401s immediately. Tightened ip-ban from fail-open to fail-closed when req.ip can't be resolved (was: ban check skipped silently; now: 400 BadRequest)." },
    ],
  },
  {
    id: "phase-47",
    title: "Phase 47 — defense polish: skeletons, ER fullscreen, TopNav, banner",
    subtitle: "Last UX pass before defense — every page reserves space, ER diagram fits the room, and the Stripe banner finally goes away when it should.",
    icon: Sparkles,
    tone: "yellow",
    shippedAt: "May 2, 2026",
    commitSha: "9ec9b04",
    items: [
      { title: "25 new loading.tsx skeletons across admin (8), seller (12), and public (5) pages. Every route now mirrors its live layout exactly so navigation no longer flashes a blank frame or layout-shifts when the data lands." },
      { title: "ER diagram now drags from anywhere on the canvas, not just the gaps between cards. Native HTML5 image-drag was hijacking pointer events so the user kept ghost-dragging entity cards instead of panning. draggable={false} + user-select: none on every card kills the ghost." },
      { title: "ER diagram fullscreen mode — new Maximize2 button + Ctrl/Cmd+Enter shortcut. Uses the Fullscreen API on the canvas container so the diagram fills the viewport for screenshots and live walkthroughs." },
      { title: "TopNav category chips now point at real DB category IDs (1-10). Prior values 11/17/19/20 were invented IDs that didn't exist, so every chip routed to an empty /browse?category=XX. Reordered alphabetically + relabelled with canonical names so TopNav and the /browse left sidebar list the same categories in the same order." },
      { title: "Stripe-onboarding banner self-fetches /api/seller/stripe/status on mount AND on every pathname change. Layout cache used to freeze needsStripe=true so a seller who finished Connect onboarding still saw 'Set up Stripe →' until they hard-refreshed. Now the banner disappears the moment the account.updated webhook lands." },
      { title: "Pixel Forge Bangkok seed — Bank's store rebranded from 'Test store' with 30 game-dev / indie-studio products across all 10 categories and all 4 delivery methods. Companion to the Aurora Creative Lab seed; the two stores read as different studios so demo browsing is varied." },
    ],
  },
  {
    id: "phase-46",
    title: "Phase 46 — Firebase Phone Auth + Stripe Connect checkout fix",
    subtitle: "Drop-in SMS OTP path via Firebase + the embedded checkout finally renders the card form.",
    icon: ShieldAlert,
    tone: "info",
    shippedAt: "May 1, 2026",
    commitSha: "628af5a",
    items: [
      { title: "Stripe checkout PaymentElement was failing to mount with 'Unhandled payment Element loaderror' — direct-charge intents live on the seller's Connect account, so loadStripe() needs the stripeAccount option. The page now reads the seller's stripeAccountId via the order's first item and passes it through; Stripe.js scopes correctly + the card form renders." },
      { title: "Firebase Phone Auth groundwork: server-side firebase-admin lib, POST /auth/verify-phone-firebase endpoint, client-side firebase init, FirebasePhoneVerify component with invisible reCAPTCHA + signInWithPhoneNumber + confirm flow. Posts the resulting ID token to the BFF which verifies it server-side and stamps phoneVerifiedAt." },
      { title: "CSP additions for Firebase + reCAPTCHA: identitytoolkit.googleapis.com, securetoken.googleapis.com, www.googleapis.com (connect-src); www.gstatic.com, www.google.com, apis.google.com (script-src); reCAPTCHA challenge frame (frame-src). Spark plan = 10 free SMS/day, pay-as-you-go after that." },
      { title: "Firebase is opt-in. /verify-phone keeps using the in-house OTP flow (DEMO_REVEAL_TOKENS surfaces the code on screen for defense) until you wire NEXT_PUBLIC_FIREBASE_API_KEY etc. on Fly + paste the service-account JSON for the API." },
    ],
  },
  {
    id: "phase-45",
    title: "Phase 45 — schema alignment with the submitted CPE241 docx report",
    subtitle: "Adjust schema + code so what runs in prod matches the ER diagram in the report submitted to the examiner.",
    icon: Database,
    tone: "purple",
    shippedAt: "May 1, 2026",
    commitSha: "b1f6c84",
    items: [
      { title: "Product owns deliveryMethod + isStackable now (mirrors first variant on create). ProductItem.deliveryMethod stays for back-compat." },
      { title: "ProductItem.name is required (NOT NULL VARCHAR(100)) — backfilled from the product name; '<product> — Variant N' for multi-variant products. ProductItem.description + image added (nullable)." },
      { title: "Order.userId is now a direct FK denormalised from Cart.userId — backfilled in the migration, set on every Order create. Reports can join Order → User without the cart hop." },
      { title: "OrderItem.priceAtPurchase renamed to pricePerUnit, widened to Decimal(12,2)." },
      { title: "New ProductDetail table for the report's 'Product Additional Detail' rows (max 7 enforced at the app layer)." },
      { title: "TransactionType enum drops 'refund'; existing refund rows convert to payout with negative amount. refundOrder + refundTransaction services updated to log payouts." },
      { title: "Cart 'Add to cart' then 'Buy now' on a digital product made qty=2 even though the frontend caps digital at 1. Backend now enforces the digital-cap-at-1 + stock-cap on every merge so qty never exceeds the buyer's check-out ceiling." },
      { title: "Cart trash icon failed silently on any non-200; now surfaces the per-line error (network, 401, 404) and refreshes when emptied so the EmptyState card replaces the stranded summary." },
    ],
  },
  {
    id: "phase-44",
    title: "Phase 44 — admin polish + cart cache + Google sign-up nudge",
    subtitle: "Dropdown flips up, admin tables refresh on action, and Google-only users see a 'Set a password' banner up top.",
    icon: Wrench,
    tone: "info",
    shippedAt: "May 1, 2026",
    commitSha: "dd14360",
    items: [
      { title: "Action-row dropdown overflowed past the viewport on the last row of /admin/users + /admin/stores. Now measures spaceBelow on open + on scroll/resize and flips above the trigger when there's not enough room." },
      { title: "'Remove user' on /admin/users wrote the audit log row but the table kept showing the user. All admin-row actions (role change, force-reset toggle, remove user, suspend/delete store) now bump a `_t` cache-bust query param + use location.replace() so every cache treats the post-action page as a brand new URL." },
      { title: "Confirm-action modal text now centres horizontally — was left-aligned even though the buttons sat right." },
      { title: "/admin/users used to show a seller's deleted store in the Store column. listUsers now selects store.deletedAt and nulls the relation when the store is soft-deleted, so the column reads '—' the moment the store is dropped." },
      { title: "Google-only users couldn't tell the marketplace had a 'set a password' flow — the form lived halfway down /profile/edit. New yellow banner at the top with an in-page anchor scrolls them to the existing form." },
      { title: "TH phone field on /register now forces exactly 9 digits with no leading 0 (typing 0xxxxxxxx blocks the 0). Stored as +66xxxxxxxxx so reports stay consistent." },
      { title: "Hide 'Set up Stripe →' banner on /seller/onboarding (it was showing on the destination page itself, so the CTA was a no-op)." },
      { title: "/seller/stripe/status now returns configured:true on the success path. Was returning only the account flags so configured resolved to undefined and the page mistakenly told sellers Stripe wasn't set up." },
      { title: "BFF /api/cart used to return stale snapshots even after writes — added explicit cache: 'no-store' on every BFF→API fetch so the proxy never reuses Next's data cache." },
    ],
  },
  {
    id: "phase-43",
    title: "Phase 43 — Google sign-in survives, OAuth gets a userId",
    subtitle: "Round-2 defense bug sweep. Fixes the OAuth account-link flow + half a dozen other production bugs found during the live test.",
    icon: ShieldAlert,
    tone: "danger",
    shippedAt: "May 1, 2026",
    commitSha: "7cb5870",
    items: [
      { title: "Google sign-in for fresh users: declared additionalFields in better-auth's user model so the user.create.before hook's firstName/lastName/username actually reach Prisma. Was getting filtered to {} → 'unable_to_create_user' callback." },
      { title: "Mirror userId↔id on every Prisma proxy result (create + update + upsert), so OAuth account-link gets a userId. Was previously only mirroring on read." },
      { title: "Strip Domain= attribute when mirroring API Set-Cookie through the BFF — 'state_mismatch' on Google callback was a sibling-domain cookie problem (API at metu-api.fly.dev sets Domain=metu-api.fly.dev which the BFF at metu.fly.dev refuses)." },
      { title: "/browse falls back to ALL stores when no store has Stripe ready (rather than empty grid). Defense scenarios where Stripe onboarding hasn't completed don't strand the marketplace." },
      { title: "Register controller forwards the in-progress demo payload (OTP + email-verify token) to the verify pages so DEMO_REVEAL_TOKENS works end-to-end during defense." },
      { title: "Cart line images load eagerly. Lazy-loaded 80x80 thumbnails were leaving empty boxes when the IntersectionObserver didn't fire (small viewport box, full-page paint already complete)." },
      { title: "Tech-stack logo lazy-load fix + cart qty label tweak + Google new-user verify-phone bounce." },
    ],
  },
  {
    id: "phase-42",
    title: "Phase 42 — pre-defense bug sweep",
    subtitle: "Real bug-fix pass after a hands-on test session. Twelve issues, all live.",
    icon: Bug,
    tone: "success",
    shippedAt: "May 1, 2026",
    commitSha: "748c9a8",
    items: [
      { title: "Store owners can't buy from their own store. Frontend hides the buy buttons + shows a 'this is your own store' notice; the cart API rejects with CannotBuyOwnProduct as a second line of defence." },
      { title: "Browse only shows products from stores that have completed Stripe onboarding. Stops the silent demo-mode checkout that was eating buyers' clicks." },
      { title: "Sellers can finally enter the post-purchase delivery URL + license-key template per variant on /seller/products/new and the edit page. Conditional UI: download/streaming gets a URL field, license_key/email gets a template." },
      { title: "Seed migration backfills sampleUrl + deliveryUrl + licenseKeyTemplate on every old product so /product/<id> stops hiding the Free sample link." },
      { title: "Validation errors no longer dump raw Zod JSON. New zod-humanise helper turns 'too_small / minimum 2' into 'Description must be at least 2 characters' and the like." },
      { title: "Email no longer rides in URL query strings. /verify-pending and /verify-phone read from a signed metu_pv cookie set by the BFF on register / blocked-login. Drops a real referrer/log leak." },
      { title: "Reset-password validity check is POST now, not GET. Token stays in the body, never lands in BFF or Fly access logs. TokenScrubber strips ?token= from the address bar after the page consumes it." },
      { title: "Profile shows red/yellow banners when email or phone aren't verified, with one-click 'Resend link' / 'Verify now'. No more silent half-verified accounts." },
      { title: "Google sign-in actually works. The blanket dup-check that threw 'EmailAlreadyRegistered' on every sign-up got removed and replaced with better-auth's accountLinking on the google provider." },
      { title: "Shared GoogleSignInButton on /login, /register, and the profile 'Connected accounts' card. Old anchors did GET → 405; new component POSTs to /sign-in/social and follows the returned URL." },
      { title: "Phone field has a country picker now (TH / SG / MY / ID / PH / VN / JP / KR / US / GB). Defaults from cf-ipcountry when present, then accept-language, then TH. Strips the leading 0 for Thai numbers automatically." },
      { title: "Dialog body wraps long text. Suspend-store / delete-store confirms used to spill outside the dialog frame." },
      { title: "Admin /stores rows now show a coral 'Suspended' or grey 'Deleted' badge so the action's effect is visible without checking the audit log." },
    ],
  },
  {
    id: "phase-41",
    title: "Phase 41 — mandatory verify at register",
    subtitle: "Email link + phone OTP both required before login unlocks. No more skip path.",
    icon: KeyRound,
    tone: "warning",
    shippedAt: "May 1, 2026",
    commitSha: "d6dd22b",
    items: [
      { title: "Register now demands a phone number, generates a one-time email-verify token, and an SMS OTP." },
      { title: "Login throws 403 EmailNotVerified or PhoneNotVerified until both are confirmed; the form bounces to the matching verify page." },
      { title: "Email verify uses a SHA-256-hashed single-use token in EmailVerifyToken; phone OTP uses a hashed code with a 10-minute TTL on the user row." },
      { title: "Reset-password links now expire after 5 minutes (was 30) and the page re-validates server-side so a stale link can never load the form." },
      { title: "Branded HTML email layout: gold logo card, mint accent button, plain-text fallback. Resend handles delivery; missing API key falls back to console for local dev." },
      { title: "Migration phase_41_verify_at_register adds phoneOtpHash, phoneOtpExpiresAt, and the EmailVerifyToken table. Existing seed accounts auto-marked verified so the gate doesn't strand them." },
    ],
  },
  {
    id: "phase-16-3",
