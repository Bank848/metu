...
export function BulkEditForm({ items }: { items: Item[] }) {
  ...
  // Open the in-page confirm modal before fanning out the price patch.
            ...
            {items.map((it) => {
              ...
              return (
                <tr key={it.productItemId} className={cn(!it.isActive && "opacity-60")}>
                  ...
                  <td className="px-3 py-3 text-right">{coinsOrFree(thbToCoins(it.price))}</td>
                  <td
                    className={cn(
                  ...
                  >
                    {coinsOrFree(thbToCoins(newPrice))}
                  </td>
                </tr>
              );
            })}
